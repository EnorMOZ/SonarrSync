    ...
    for name, server in servers.items():
        if movie['profileId'] == int(server['profileidmatch']):
            if movie['tmdbId'] not in server['movies']:
                spath = ConfigSectionMap("RadarrMaster")['basepath']
                logging.debug('spath: {0}'.format(spath))
                if spath in movie['path']:
                    rpath = str.replace(str(movie['path']), ConfigSectionMap("RadarrMaster")['basepath'], server['newpath'])
